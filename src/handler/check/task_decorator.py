...
def check_task(name: str, description: str, 
    ...
    def decorator(cls):
        ...
        def wrapped_execute(self):
            ...
            if meta['min_ob_version']:
                if not self.check_ob_version_min(meta['min_ob_version']):
                    self.report.add_skip(
                        f"Requires OB >= {meta['min_ob_version']}, "
                        f"current: {self.observer_version}"
                    )
            ...
            if meta['max_ob_version']:
                ...
                if self.observer_version and StringUtils.compare_versions_greater(
                    self.observer_version, meta['max_ob_version']
                ):
                    self.report.add_skip(
                        f"Requires OB <= {meta['max_ob_version']}, "
                        f"current: {self.observer_version}"
                    )
                    return TaskResult.SKIPPED
                ...
                if hasattr(self, 'observer_nodes') and self.observer_nodes:
                    ...
                    if ssh_client:
                        ...
                        if os_type not in meta['supported_os']:
                            self.report.add_skip(
                                f"OS '{os_type}' not supported. Supported: {meta['supported_os']}"
                            )
                            return TaskResult.SKIPPED
                    ...
                    for node in self.observer_nodes:
                        if node.get("ssher") is None:
                            self.report.add_fail(
                                f"SSH not available for node {node.get('ip', 'unknown')}"
                            )
                            return TaskResult.FAILED
