...
class HandlerInvoker:
    ...
    def _initialize_managers(self):
        ...
        try:
            ...
            self.config_manager = ConfigManager(config_path=self.config_path, stdio=self.stdio, inner_config_manager=self.inner_config_manager)

            # Initialize context manager
            self.context_manager = ContextManager(stdio=self.stdio, config_manager=self.config_manager, inner_config_manager=self.inner_config_manager)
    ...
    def invoke(self, tool_name: str, arguments: Dict[str, Any], capture_output: bool = True) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            context = self.context_manager.create_context(handler_name=handler_name, namespace=namespace, options=options, skip_cluster_conn=False)  # Most handlers need cluster connection
            ...
            # Create handler
            handler = self.handler_factory.create(handler_name=handler_name, context=context)
