...
class ConfigHelper(object):
    ...
    def build_configuration_by_ini(self, ini_file_path="conf/configure.ini"):
        ...
        print(ini_file_path)
        print(type(ini_file_path))

        self.stdio.print("inpi_file_path: {0}".format(os.path.expanduser(ini_file_path)))
        ini_file_path = os.path.expanduser(ini_file_path)
        ...
        if len(observer_hosts) == 0:
            ...
            for observer_host in observer_hosts:
                ...
                sshhelper = SshClient(context=self.context, node=node)
                print(type(sshhelper))
                all_dir_name = sshhelper.exec_cmd("ls {0}".format(work_dir))
                print(all_dir_name)
                all_dir = all_dir_name.split("\n")
            ...
            for obproxy_host in obproxy_hosts:
                ...
                for dir_name in all_dir_name:
                    if "obproxy" in dir_name:
                        ...
                        continue
        print("config_yml_obproxy: {0}".format(config_yml_obproxy))
        print("config_yml_obcluster: {0}".format(config_yml_obcluster))
        # config_yml_obcluster
        # db_host
